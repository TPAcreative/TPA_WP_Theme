 ...
 * @since     2.0
 */
if ( ! function_exists( 'ot_type_country' ) ) {
  
  function ot_type_country( $args = array() ) {
    ...
    echo '<div class="format-setting type-select ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '">';
      ...
      echo '<div class="format-setting-inner">';
        ...
        foreach($country_list as $code => $country){
           echo '<option value="'.$code.'" '.selected( $field_value, $code, false ).'>'.$country.'</option>';
        }
 ...
 * @since     2.0
 */
if ( ! function_exists( 'ot_type_sidebar' ) ) {
  
  function ot_type_sidebar( $args = array() ) {
    ...
    echo '<div class="format-setting type-select ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '">';
      ...
      echo '<div class="format-setting-inner">';
        ...
        $sidebars = ot_get_option('indigo_create_sidebars');

        // If there's a value for a new sidebar
        if(!empty($sidebars)){
          // Loop through array to register each sidebar
          foreach($sidebars as $sidebar){
            echo '<option value="'.esc_attr($sidebar['title']).'" '.selected( $field_value, esc_attr($sidebar['title']), false ).'>'.$sidebar['title'].'</option>';
          } 
        }
